...
# Paths
PYTHON_UPDATES_DIR="/var/local/lib/updates"
    ...
    case "$operation" in
        "enable")
            if [ -n "$component" ]; then
                ...
                python3 "$PYTHON_ORCHESTRATOR" --enable-component "$target" "$component"
            else
                log_message "Enabling module '$target'..."
                python3 "$PYTHON_ORCHESTRATOR" --enable-module "$target"
            fi
            ;;
        "disable")
            if [ -n "$component" ]; then
                log_message "Disabling component '$component' in module '$target'..."
                python3 "$PYTHON_ORCHESTRATOR" --disable-component "$target" "$component"
            else
                log_message "Disabling module '$target'..."
                python3 "$PYTHON_ORCHESTRATOR" --disable-module "$target"
            fi
            ;;
        "list")
            log_message "Listing all modules..."
            python3 "$PYTHON_ORCHESTRATOR" --list-modules
            ;;
        "status")
            if [ -n "$target" ]; then
                log_message "Getting status for module '$target'..."
                python3 "$PYTHON_ORCHESTRATOR" --module-status "$target"
            else
                log_message "Getting status for all modules..."
                python3 "$PYTHON_ORCHESTRATOR" --all-status
            fi
            ;;
    esac
