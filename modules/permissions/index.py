...
def ensure_repo_checkout(module_dir: Path, repo_cfg: Dict[str, Any]) -> Path:
    ...
    try:
        if not cache_dir.exists():
            log_message(f"[PERMISSIONS] Cloning repository {repo_url} (branch: {branch}) to {cache_dir}")
            ...
            log_message(f"[PERMISSIONS] Fetching updates from {repo_url} (branch: {branch})")
        ...
        log_message(f"[PERMISSIONS] git repo sync failed: {e.stderr}", 'ERROR')
        return module_dir / 'src'
    return cache_dir
...
def _check_version_update_needed() -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            log_message(f"[PERMISSIONS] Cloning repository for version check: {repo_cfg['url']} (branch: {repo_cfg.get('branch', 'master')})")
            env = os.environ.copy()
            env['GIT_TERMINAL_PROMPT'] = '0'
