#!/usr/bin/env python3
"""
Keyman Module - Manage /vault/keyman credential tools via manifest
"""
import os
import json
import hashlib
...
def _dest_path(entry: Dict[str, Any]) -> Path:
    dest = entry.get('destination') or f"/vault/keyman/{entry['source']}"
    return Path(dest)
...
def check(module_dir: Path) -> Dict[str, Any]:
    def write_state(state: Dict[str, Any]) -> None:
        try:
            ...
            log_message(f"Failed to write keyman state: {e}", 'WARNING')
    ...
    log_message(f"Keyman module status: {len(enabled_files)} enabled files")
...
def apply(module_dir: Path) -> Dict[str, Any]:
    ...
    perm_mgr = PermissionManager('keyman')
    changed: List[str] = []

    # Check if update is needed first
    update_needed, remote_version = _check_version_update_needed()
    if not update_needed:
        log_message("Keyman module is already up to date, no update needed")
        return {
            'success': True, 
            'updated': False, 
            'changed': [],
    ...
    state.backup_module_state('keyman', description='keyman credential tools', files=backup_targets)
    ...
    for entry in files:
        ...
        perm_mgr.set_permissions([PermissionTarget(tmp_path.as_posix(), entry.get('owner','root'), entry.get('group','root'), int(entry.get('mode','700'),8))])
        os.replace(tmp_path, dest)
        changed.append(str(dest))
        log_message(f"Updated keyman tool: {dest}")
