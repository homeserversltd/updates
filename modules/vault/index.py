#!/usr/bin/env python3
"""
Vault Module - Manage /vault/scripts via manifest
"""
import os
import json
import hashlib
...
def ensure_repo_checkout(module_dir: Path, repo_cfg: Dict[str, Any]) -> Path:
    ...
    try:
        if not cache_dir.exists():
            log_message(f"[VAULT] Cloning repository {repo_url} (branch: {branch}) to {cache_dir}")
            ...
            log_message(f"[VAULT] Fetching updates from {repo_url} (branch: {branch})")
        ...
        log_message(f"[VAULT] git repo sync failed: {e.stderr}", 'ERROR')
        return module_dir / 'src'
    return cache_dir
...
def _dest_path(entry: Dict[str, Any]) -> Path:
    dest = entry.get('destination') or f"/vault/scripts/{entry['source']}"
    return Path(dest)
...
def check(module_dir: Path) -> Dict[str, Any]:
    def write_state(state: Dict[str, Any]) -> None:
        try:
            ...
            log_message(f"Failed to write vault state: {e}", 'WARNING')
    ...
    log_message(f"Vault module status: {len(enabled_files)} enabled files")
...
def _check_version_update_needed() -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        temp_dir = tempfile.mkdtemp(prefix="vault_version_check_")
        try:
            # Shallow clone just for version check
            log_message(f"[VAULT] Cloning repository for version check: {repo_cfg['url']} (branch: {repo_cfg.get('branch', 'master')})")
            env = os.environ.copy()
            env['GIT_TERMINAL_PROMPT'] = '0'
...
def apply(module_dir: Path) -> Dict[str, Any]:
    ...
    perm_mgr = PermissionManager('vault')
    changed: List[str] = []

    # Check if update is needed first
    update_needed, remote_version = _check_version_update_needed()
    if not update_needed:
        log_message("Vault module is already up to date, no update needed")
        return {
            'success': True, 
            'updated': False, 
            'changed': [],
    ...
    state.backup_module_state('vault', description='vault scripts', files=backup_targets)
    ...
    for entry in files:
        ...
        log_message(f"Updated vault script: {dest}")
